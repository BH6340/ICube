    ...
    <div v-if="post">
      ...
      <el-card class="post-card" shadow="never">
        <div class="post-header">
          ...
          <div class="author-section">
            ...
            <div class="post-actions" v-if="isLoggedIn">
              ...
              <el-button :type="post.is_collected ? 'warning' : 'default'" @click="handleCollect">
                <el-icon><FolderChecked v-if="post.is_collected" /><Folder v-else /></el-icon>
                {{ post.is_collected ? '已收藏' : '收藏' }}
              </el-button>

              <el-button v-if="isOwner" @click="goToEdit" :icon="Edit"> 编辑 </el-button>
        ...
        <div class="post-stats">
          <span
            ><el-icon><View /></el-icon> {{ post.view_count }} 次浏览</span
          >
          <span
            ><el-icon><ChatLineRound /></el-icon> {{ post.comment_count }} 条评论</span
          >
          <span
            ><el-icon><Star /></el-icon> {{ post.like_count }} 人点赞</span
          >
        </div>
      ...
      <el-card class="comments-card" shadow="never">
        <template #header>
          <div class="comments-header">
            <h3>
              评论 <span class="comment-count">({{ post.comment_count }})</span>
            </h3>
          </div>
        </template>
