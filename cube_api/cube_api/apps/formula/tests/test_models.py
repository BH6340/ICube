...
class CubeCategoryModelTest(TestCase):
    ...
    def setUp(self):
        """创建测试数据"""
        self.category_data = {"order": 3, "method": "CFOP", "phase": "OLL", "name": "OLL 分类"}
    ...
    def test_custom_category_with_created_by(self):
        """测试用户自定义分类"""
        user = User.objects.create_user(email="custom@example.com", password="testpass", username="customuser")

        category = CubeCategory.objects.create(
            order=3, method="CFOP", phase="Custom", name="自定义分类", created_by=user, is_custom=True
        )
...
class CubeStateModelTest(TestCase):
    ...
    def test_state_name_uniqueness(self):
        ...
        with self.assertRaises(IntegrityError):
            CubeState.objects.create(name="OLL-01", state_definition={"order": 3, "blocks": []})

    def test_state_with_category(self):
        """测试状态关联分类"""
        category = CubeCategory.objects.create(order=3, method="CFOP", phase="OLL", name="OLL分类")
        state = CubeState.objects.create(
            name="OLL-WithCategory", state_definition=self.state_data["state_definition"], category=category
        )
...
class FormulaModelTest(TestCase):
    ...
    def setUp(self):
        """创建测试数据"""
        self.category = CubeCategory.objects.create(order=3, method="CFOP", phase="OLL", name="OLL分类")
    ...
    def test_create_formula_success(self):
        ...
        formula = Formula.objects.create(
            name="R U R'U'", notation="R U R' U'", category=self.category, target_state=self.state
        )
    ...
    def test_inverse_notation_auto_generation(self):
        """测试逆公式自动生成"""
        formula = Formula.objects.create(name="逆公式测试", notation="R U R' U'", category=self.category)
    ...
    def test_inverse_notation_handles_various_notations(self):
        ...
        for notation, expected_inverse in test_cases:
            formula = Formula.objects.create(name=f"测试-{notation}", notation=notation, category=self.category)
            self.assertEqual(
    ...
    def test_formula_str_method(self):
        """测试 __str__ 方法"""
        formula = Formula.objects.create(name="测试公式", notation="R U R' U'", category=self.category)
        self.assertEqual(str(formula), "3阶 CFOP OLL - 测试公式")

    def test_formula_view_count_default(self):
        """测试浏览量默认为 0"""
        formula = Formula.objects.create(name="浏览量测试", notation="R U", category=self.category)
        self.assertEqual(formula.view_count, 0)

    def test_custom_formula_with_user(self):
        """测试用户自定义公式"""
        user = User.objects.create_user(
            email="formula_creator@example.com", password="testpass", username="formula_creator"
        )

        formula = Formula.objects.create(
            name="自定义公式", notation="R' F R F'", category=self.category, is_custom=True, created_by=user
        )
    ...
    def test_get_pre_state_with_pre_state_definition(self):
        ...
        formula = Formula.objects.create(
            name="前置状态测试", notation="R U", category=self.category, pre_state_definition=pre_state
        )
    ...
    def test_get_pre_state_from_target_state(self):
        """测试从目标状态推导前置状态"""
        formula = Formula.objects.create(
            name="推导前置状态", notation="R U", category=self.category, target_state=self.state
        )
    ...
    def test_get_pre_state_without_any_state(self):
        """测试无任何状态时返回 None"""
        formula = Formula.objects.create(name="无状态公式", notation="R", category=self.category)
...
class FormulaTagRelationModelTest(TestCase):
    ...
    def setUp(self):
        """创建测试数据"""
        self.category = CubeCategory.objects.create(order=3, method="CFOP", phase="OLL", name="测试分类")
        self.formula = Formula.objects.create(name="关联测试公式", notation="R U", category=self.category)
        self.tag = FormulaTag.objects.create(name="测试标签")

    def test_create_relation_success(self):
        """测试创建关联成功"""
        relation = FormulaTagRelation.objects.create(formula=self.formula, tag=self.tag)
...
class FormulaCollectionModelTest(TestCase):
    ...
    def setUp(self):
        """创建测试数据"""
        self.user = User.objects.create_user(email="collector@example.com", password="testpass", username="collector")
        self.category = CubeCategory.objects.create(order=3, method="CFOP", phase="OLL", name="收藏测试分类")
        self.formula = Formula.objects.create(name="收藏测试公式", notation="R U R' U'", category=self.category)

    def test_create_collection_success(self):
        """测试创建收藏成功"""
        collection = FormulaCollection.objects.create(user=self.user, formula=self.formula)
    ...
    def test_multiple_collections(self):
        """测试收藏多个公式"""
        formula2 = Formula.objects.create(name="第二个公式", notation="F R", category=self.category)
    ...
    def test_get_or_create_idempotent(self):
        ...
        collection1, created1 = FormulaCollection.objects.get_or_create(user=self.user, formula=self.formula)
        self.assertTrue(created1)

        collection2, created2 = FormulaCollection.objects.get_or_create(user=self.user, formula=self.formula)
