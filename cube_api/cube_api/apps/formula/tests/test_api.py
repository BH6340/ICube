...
from apps.formula.models import Formula, FormulaCollection
from django.contrib.auth import get_user_model
from rest_framework import status
from rest_framework.test import APIClient

from .base import FormulaBaseTestCase
...
class CubeStateAPITest(FormulaBaseTestCase):
    ...
    def test_create_state_as_admin(self):
        ...
        data = {"name": "新状态", "state_definition": self._get_valid_state_definition(), "category": self.category.id}
...
class FormulaAPITest(FormulaBaseTestCase):
    ...
    def test_filter_custom_formulas_by_author_username_anonymously(self):
        ...
        response = self.client.get(f"/api/formula/formulas/?author_username={self.user.username}&is_custom=true")

        ids = [item["id"] for item in response.data["data"]["results"]]
        self.assertEqual(ids, [custom.id])
        self.assertTrue(all(item["is_custom"] for item in response.data["data"]["results"]))
...
class FormulaCollectionAPITest(FormulaBaseTestCase):
    ...
    def test_public_user_collections_are_readable_anonymously(self):
        ...
        response = APIClient().get(f"/api/formula/collections/users/{self.user.username}/")
    ...
    def test_public_user_collections_return_not_found_for_missing_user(self):
        ...
        response = APIClient().get("/api/formula/collections/users/missing_user/")
