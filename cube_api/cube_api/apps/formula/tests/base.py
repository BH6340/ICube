...
class FormulaBaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user = self.create_test_user(
            email="formula_test@example.com", password="test123456", username="formula_tester"
        )

        # 创建管理员用户
        self.admin_user = self.create_test_admin(
            email="formula_admin@example.com", password="admin123456", username="formula_admin"
        )

        # 创建测试分类
        self.category = self.create_test_category(order=3, method="CFOP", phase="OLL", name="OLL 分类")

        # 创建测试状态
        self.state = self.create_test_state(
            name="OLL-01", state_definition=self._get_valid_state_definition(), category=self.category
        )

        # 创建测试公式
        self.formula = self.create_test_formula(
            name="测试公式", notation="R U R' U'", category=self.category, target_state=self.state
        )
    ...
    def create_test_admin(self, email, password, username, **kwargs):
        """创建管理员测试用户"""
        return User.objects.create_superuser(email=email, password=password, username=username, **kwargs)

    def create_test_category(self, order=3, method="CFOP", phase="OLL", name="测试分类", **kwargs):
        """创建测试分类"""
        return CubeCategory.objects.create(order=order, method=method, phase=phase, name=name, **kwargs)
    ...
    def create_test_state(self, name="测试状态", state_definition=None, category=None, **kwargs):
        ...
        return CubeState.objects.create(name=name, state_definition=state_definition, category=category, **kwargs)
