...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("json_file", type=str, help="公式 JSON 文件路径")
        parser.add_argument("--force", action="store_true", help="已存在同名称公式时，删除后重新导入")
    ...
    def handle(self, *args, **options):
        ...
        for formula_data in formulas:
            ...
            if category_name:
                ...
                if not category:
                    self.stdout.write(self.style.WARNING(f"  分类不存在: {category_name}，跳过此公式"))
                    warned_count += 1
                    continue
