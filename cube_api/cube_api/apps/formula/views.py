...
from .models import CubeCategory, CubeState, Formula, FormulaCollection, FormulaTag
from .permissions import IsAdminOrCustomCreator, IsAdminOrReadOnly
from .serializers import (
    CubeCategorySerializer,
    CubeStateSerializer,
    FormulaListSerializer,
    FormulaMatchSerializer,
    FormulaSerializer,
    FormulaSimpleSerializer,
    FormulaTagSerializer,
)
...
class CubeCategoryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated:
            return CubeCategory.objects.filter(models.Q(created_by__isnull=True) | models.Q(created_by=user))
    ...
    def my_custom(self, request):
        ...
        """
        categories = CubeCategory.objects.filter(created_by=request.user, is_custom=True)
...
class FormulaViewSet(viewsets.ModelViewSet):
    ...
    queryset = Formula.objects.select_related("category", "target_state", "created_by").prefetch_related(
        "tag_relations__tag"
    )
    ...
    @extend_schema(
        ...
        responses={
            200: {"type": "object", "properties": {"formulas": {"type": "array", "description": "匹配的公式列表"}}}
        },
    )
    ...
    @extend_schema(
        ...
        responses={200: {"type": "object", "properties": {"authors": {"type": "array", "description": "作者列表"}}}},
    )
    ...
    def authors(self, request):
        ...
        """
        authors = (
            Formula.objects.filter(created_by__isnull=False)
            .values("created_by__id", "created_by__username")
            .distinct()
        )

        author_list = [
            {"id": author["created_by__id"], "username": author["created_by__username"]} for author in authors
        ]
...
class FormulaCollectionViewSet(viewsets.ModelViewSet):
    ...
    def user_collections(self, request, username=None):
        ...
        collections = (
            FormulaCollection.objects.filter(
                user=user,
            )
            .select_related(
                "formula__category",
    ...
    def create(self, request, *args, **kwargs):
        ...
        _collection, created = FormulaCollection.objects.get_or_create(user=request.user, formula=formula)
