    ...
    def import_tags(self):
        ...
        for tag_data in tags_data:
            tag, created = Tag.objects.get_or_create(
                name=tag_data["name"], defaults={"color": tag_data["color"], "use_count": tag_data["use_count"]}
            )
            if created:
                created_count += 1
    ...
    def create_test_users(self):
        ...
        for user_data in users_data:
            user, created = User.objects.get_or_create(
                email=user_data["email"], defaults={"username": user_data["username"], "bio": "魔方爱好者，欢迎交流！"}
            )
            if created:
