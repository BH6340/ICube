...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("data_dir", type=str, help="数据包目录路径（包含 articles.json 和 images/）")
        parser.add_argument("--author-id", type=int, help=f"指定作者 ID（默认使用 {DEFAULT_AUTHOR_EMAIL}）")
        parser.add_argument("--force", action="store_true", help="已存在同标题文章时，删除后重新导入")
    ...
    def handle(self, *args, **options):
        ...
        for article_data in articles:
            ...
            for tag_name in tags:
                tag, _ = Tag.objects.get_or_create(name=tag_name, defaults={"color": "#1890ff"})
                post.tags.add(tag)
            ...
            if path_map:
                ...
                for original, actual in path_map.items():
                    ...
                    new_content_md = new_content_md.replace(f"]({original})", f"]({media_path})")
    ...
    def _resolve_author(self, author_id):
        ...
        if not author:
            self.stdout.write(self.style.ERROR(f"默认作者 {DEFAULT_AUTHOR_EMAIL} 不存在，请用 --author-id 指定"))
        return author
