...
from apps.forum.models import Comment, CommentLike, Post, PostCollect, PostLike, Tag
...
class PostModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.tag = Tag.objects.create(name="CFOP", color="#409EFF")
        self.post = Post.objects.create(title="测试帖子", content="这是测试内容", author=self.user)
...
class CommentModelTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email="test@example.com", password="test123", username="testuser")
        self.post = Post.objects.create(title="测试帖子", content="测试内容", author=self.user)
        self.comment = Comment.objects.create(post=self.post, author=self.user, content="测试评论")
    ...
    def test_reply_comment(self):
        """测试回复评论"""
        reply = Comment.objects.create(post=self.post, author=self.user, parent=self.comment, content="回复内容")
...
class InteractionModelTest(TestCase):
    ...
    def setUp(self):
        self.user1 = User.objects.create_user(email="user1@example.com", password="test123", username="user1")
        self.user2 = User.objects.create_user(email="user2@example.com", password="test123", username="user2")
        self.post = Post.objects.create(title="测试帖子", content="测试内容", author=self.user1)
        self.comment = Comment.objects.create(post=self.post, author=self.user1, content="测试评论")
    ...
    def test_post_collect(self):
        """测试帖子收藏"""
        PostCollect.objects.create(post=self.post, user=self.user2)
        self.assertEqual(PostCollect.objects.count(), 1)

    def test_comment_like_dislike(self):
        """测试评论点赞/点踩"""
        like = CommentLike.objects.create(comment=self.comment, user=self.user2, is_like=True)
        self.assertTrue(like.is_like)

        dislike = CommentLike.objects.create(comment=self.comment, user=self.user1, is_like=False)
