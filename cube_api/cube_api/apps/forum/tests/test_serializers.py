...
class PostSerializerTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email="test@example.com", password="test123", username="testuser")
        self.post = Post.objects.create(title="测试帖子", content="测试内容", author=self.user)
        self.serializer = PostSerializer(instance=self.post, context={"request": None})
...
class PostCreateUpdateSerializerTest(TestCase):
    ...
    def test_validate_title(self):
        """测试标题验证"""
        serializer = PostCreateUpdateSerializer(data={"title": "短", "content": "测试内容"})
        self.assertFalse(serializer.is_valid())
        self.assertIn("title", serializer.errors)

    def test_validate_content(self):
        """测试内容验证"""
        serializer = PostCreateUpdateSerializer(data={"title": "有效标题", "content": "短"})
        self.assertFalse(serializer.is_valid())
