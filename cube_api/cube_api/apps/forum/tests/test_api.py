...
class ForumAPITest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            email="apitester@example.com", password="test123456", username="apitester"
        )
    ...
    def test_create_and_get_post(self):
        ...
        create_res = self.client.post(
            "/api/forum/posts/",
            {"title": "API测试帖子标题", "content": "API测试帖子内容内容内容", "tag_ids": [self.tag.id]},
            format="json",
        )
    ...
    def test_list_posts(self):
        ...
        # 创建几个帖子
        for i in range(3):
            Post.objects.create(title=f"列表测试帖子{i}", content="测试内容内容内容", author=self.user)
    ...
    def test_list_posts_filters_by_author_username(self):
        ...
        response = self.client.get(f"/api/forum/posts/?author_username={self.user.username}")
    ...
    def test_like_post(self):
        """测试点赞帖子"""
        post = Post.objects.create(title="点赞测试帖子", content="测试内容内容内容", author=self.user)
