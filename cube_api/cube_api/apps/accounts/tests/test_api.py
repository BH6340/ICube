...
class AuthAPITest(AccountsBaseTestCase):
    ...
    def test_register_success(self):
        """测试用户注册成功"""
        data = {"user": {"email": "newuser@example.com", "password": "newpass123456", "username": "newuser"}}
    ...
    def test_register_with_duplicate_email_fails(self):
        """测试重复邮箱注册失败"""
        # 先创建一个用户
        User.objects.create_user(email="duplicate@example.com", password="testpass", username="existinguser")

        # 使用相同邮箱注册
        data = {"user": {"email": "duplicate@example.com", "password": "newpass123456", "username": "newuser"}}
        response = self.client.post("/api/users/register/", data, format="json")
    ...
    def test_register_with_duplicate_username_auto_suffix(self):
        """测试用户名重复时自动添加后缀"""
        # 先创建一个用户
        User.objects.create_user(email="user1@example.com", password="testpass", username="sameusername")

        # 使用相同用户名注册，应该自动添加后缀
        data = {"user": {"email": "user2@example.com", "password": "newpass123456", "username": "sameusername"}}
        response = self.client.post("/api/users/register/", data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_register_rejects_username_with_slash(self):
        ...
        data = {
            "user": {"email": "invalid-username@example.com", "password": "newpass123456", "username": "invalid/name"}
        }
    ...
    def test_login_success(self):
        """测试用户登录成功"""
        data = {"user": {"email": "test@example.com", "password": "test123456"}}
    ...
    def test_login_with_wrong_password_fails(self):
        """测试错误密码登录失败"""
        data = {"user": {"email": "test@example.com", "password": "wrongpassword"}}
        response = self.client.post("/api/users/login/", data, format="json")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_login_with_nonexistent_email_fails(self):
        """测试不存在的邮箱登录失败"""
        data = {"user": {"email": "nonexistent@example.com", "password": "testpass"}}
        response = self.client.post("/api/users/login/", data, format="json")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class UserProfileAPITest(AccountsBaseTestCase):
    ...
    def test_update_user_info_success(self):
        """测试更新用户信息成功"""
        data = {"username": "updateduser", "bio": "这是更新后的个人简介"}
...
class ProfileDetailAPITest(AccountsBaseTestCase):
    ...
    def test_search_profiles_prioritizes_case_insensitive_exact_match(self):
        ...
        response = self.client.get(f"/api/profiles/?search={self.user.username.upper()}")
    ...
    def test_search_profiles_excludes_inactive_users(self):
        ...
        response = self.client.get(f"/api/profiles/?search={self.user2.username}")
    ...
    def test_get_following_list_supports_pagination(self):
        ...
        response = self.client.get(f"/api/profiles/{self.user.username}/following/?page=1&page_size=20")
    ...
    def test_get_followers_list_supports_pagination(self):
        ...
        response = self.client.get(f"/api/profiles/{self.user.username}/followers/?page=1&page_size=20")
