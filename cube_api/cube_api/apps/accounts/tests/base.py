...
class AccountsBaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        # 创建测试用户
        self.user = self.create_test_user(email="test@example.com", password="test123456", username="testuser")

        # 创建第二个用户（用于关注等场景）
        self.user2 = self.create_test_user(email="test2@example.com", password="test123456", username="testuser2")

        # 创建管理员用户
        self.admin_user = self.create_test_admin(
            email="admin@example.com", password="admin123456", username="adminuser"
        )
