...
class UserModelTest(TestCase):
    ...
    def setUp(self):
        """测试前准备"""
        self.user_data = {"email": "test@example.com", "password": "testpass123456", "username": "testuser"}

    def test_create_user_success(self):
        """测试创建普通用户成功"""
        user = User.objects.create_user(email="test@example.com", password="testpass123456", username="testuser")
    ...
    def test_create_superuser_success(self):
        ...
        admin = User.objects.create_superuser(
            email="admin@example.com", password="adminpass123456", username="adminuser"
        )
    ...
    def test_create_superuser_without_is_staff_raises_error(self):
        ...
        with self.assertRaises(ValueError):
            User.objects.create_superuser(
                email="admin@example.com", password="testpass", username="admin", is_staff=False
            )

    def test_email_normalization(self):
        """测试邮箱标准化（域名部分转小写，本地部分保留原大小写）"""
        user = User.objects.create_user(email="Test@Example.COM", password="testpass", username="test")
        # Django normalize_email 只标准化域名部分
        self.assertEqual(user.email, "Test@example.com")

    def test_email_uniqueness(self):
        """测试邮箱唯一性约束"""
        User.objects.create_user(email="unique@example.com", password="testpass1", username="user1")
        with self.assertRaises(IntegrityError):
            User.objects.create_user(email="unique@example.com", password="testpass2", username="user2")

    def test_username_uniqueness(self):
        """测试用户名唯一性约束"""
        User.objects.create_user(email="user1@example.com", password="testpass", username="sameuser")
        with self.assertRaises(IntegrityError):
            User.objects.create_user(email="user2@example.com", password="testpass", username="sameuser")

    def test_password_not_stored_as_plaintext(self):
        """测试密码不以明文存储"""
        user = User.objects.create_user(email="test@example.com", password="mypassword123", username="testuser")
        # 密码应被哈希，不应该等于原始密码
    ...
    def test_user_str_method(self):
        """测试 __str__ 方法返回邮箱"""
        user = User.objects.create_user(email="str@example.com", password="testpass", username="struser")
        self.assertEqual(str(user), "str@example.com")

    def test_get_full_name_returns_username(self):
        """测试 get_full_name 返回 username"""
        user = User.objects.create_user(email="test@example.com", password="testpass", username="MyName")
        self.assertEqual(user.get_full_name(), "MyName")

    def test_get_short_name_returns_username(self):
        """测试 get_short_name 返回 username"""
        user = User.objects.create_user(email="test@example.com", password="testpass", username="ShortName")
    ...
    def test_bio_field(self):
        ...
        user = User.objects.create_user(
            email="bio@example.com", password="testpass", username="biouser", bio="这是一段个人简介"
        )
        self.assertEqual(user.bio, "这是一段个人简介")

    def test_ordering_by_date_joined(self):
        """测试默认按注册时间倒序排列"""
        user1 = User.objects.create_user(email="user1@example.com", password="testpass", username="user1")
        user2 = User.objects.create_user(email="user2@example.com", password="testpass", username="user2")
...
class UserFollowTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user3 = User.objects.create_user(email="user3@example.com", password="testpass3", username="user3")
