...
- **计时方式枚举**：`timing_mode` 取值 `manual`（手动）/ `smart`（智能魔方），旧数据默认 `manual`
- **DNF 处理**：`is_dnf=True` 的记录在 stats/trend 统计中排除，但保留在列表中
- **设备关联**：前端传 `device_id`（write_only），后端反查 `SmartCubeDevice` 并绑定，设备不属于当前用户时静默跳过
- **设备唯一约束**：`user + mac_address`，`update_or_create` 自动更新而非重复创建
- **自定义 CRUD**：TimerRecordViewSet 和 SmartCubeDeviceViewSet 的 `list`/`create`/`destroy` 均重写返回 `APIResponse`，非 DRF 默认 `Response`
- **毫秒存储**：`time_ms` 使用 `IntegerField`，避免浮点精度问题
- **向后兼容**：所有智能魔方相关字段均为 nullable
